...
def creatDirectorySpeechRec(Channels):
    today = date.today()
    DIRName = "TrainingData/Speech_Recognition"
    if os.path.exists(DIRName) == False:
        os.makedirs(DIRName)
    months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
    print(int(today.month))
    ExperimentName = str(Channels) +"Ch" + "-" + str(today.day) + "-" + months[int(today.month) - 1] + "-" + str(today.year)
    ExperimentDir = os.path.join(DIRName, ExperimentName)
    if os.path.exists(ExperimentDir) == False:
        os.mkdir(ExperimentDir)
        os.mkdir(os.path.join(ExperimentDir, "Logs"))
